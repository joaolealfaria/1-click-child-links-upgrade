define([
  ...
  function getTemplates(workItemTypes) {
    ...
    workItemTypes.forEach(function (workItemType) {
      var request = witClient.getTemplates(
        ctx.project.id,
        ctx.team.id,
        workItemType
      );
    ...
    return Q.all(requests).then(function (templateTypes) {
      var templates = [];
  ...
  function IsPropertyValid(taskTemplate, key) {
    ...
    if (taskTemplate.fields[key].toLowerCase() == '@me') {
      //current identity is handled later
      return false;
    }
    if (taskTemplate.fields[key].toLowerCase() == '@currentiteration') {
      //current iteration is handled later
      return false;
    }
  ...
  function createWorkItemFromTemplate(
    ...
    for (var key in taskTemplate.fields) {
      if (IsPropertyValid(taskTemplate, key)) {
        ...
        if (taskTemplate.fields[key] == '') {
          ...
          fieldValue = replaceReferenceToParentField(
            fieldValue,
            currentWorkItem
          );
    ...
    if (taskTemplate.fields['System.IterationPath'] == null)
      ...
      taskTemplate.fields['System.IterationPath'].toLowerCase() ==
      '@currentiteration'
    )
      workItem.push({
        op: 'add',
      ...
      .then(
        function (response) {
          ...
          if (service != null) {
            ...
            witClient.updateWorkItem(document, workItemId).then(
              function (response) {
                var a = response;
  ...
  function AddTasks(workItemId, service) {
    ...
    workClient.getTeamSettings(team).then(function (teamSettings) {
      ...
      witClient.getWorkItem(workItemId).then(function (value) {
        var currentWorkItem = value.fields;

        currentWorkItem['System.Id'] = workItemId;

        var workItemType = currentWorkItem['System.WorkItemType'];
        GetChildTypes(witClient, workItemType).then(function (childTypes) {
          if (childTypes == null) return;
          // get Templates
          getTemplates(childTypes).then(function (response) {
  ...
  function IsValidTemplateWIT(currentWorkItem, taskTemplate) {
    ...
    if (IsJsonString(JSON.stringify(jsonFilters))) {
      ...
      if (!Array.isArray(rules)) rules = new Array(rules);
      ...
      if (filters) {
        var isValid = false;
        for (var i = 0; i < filters.length; i++) {
          var found = filters[i].split(',').find(function (f) {
            return (
              f.trim().toLowerCase() ==
  ...
  function matchField(fieldName, currentWorkItem, filterObject) {
    try {
      if (currentWorkItem[fieldName] == null) return false;

      if (typeof filterObject[fieldName] === 'undefined') return false;
      ...
      var match = filterValue.some((i) => {
        return (
          currentWorkItemValue.findIndex(
            (c) => i.toLowerCase() === c.toLowerCase()
          ) >= 0
        );
      });
  ...
  function findWorkTypeCategory(categories, workItemType) {
    for (category of categories) {
      var found = category.workItemTypes.find(function (w) {
        return w.name == workItemType;
      });
      if (found != null) {
        return category;
      }
  ...
  function GetChildTypes(witClient, workItemType) {
    return witClient
      .getWorkItemTypeCategories(VSS.getWebContext().project.name)
          ...
          if (category.referenceName === 'Microsoft.EpicCategory') {
            return witClient
              ...
              .then(function (response) {
                ...
                return category.workItemTypes.map(function (item) {
                  return item.name;
                });
              });
          } else if (category.referenceName === 'Microsoft.FeatureCategory') {
            requests.push(
              witClient.getWorkItemTypeCategory(
                VSS.getWebContext().project.name,
            ...
            category.referenceName === 'Microsoft.BugCategory' &&
  ...
  function ShowDialog(message) {
    ...
    VSS.getService(VSS.ServiceIds.Dialog).then(function (dialogSvc) {
      dialogSvc.openMessageDialog(message, dialogOptions).then(
        function (dialog) {
          //
        },
        function (dialog) {
  ...
  function WriteTrace(msg) {
    console.log('1-Click Child-Links-Upgrade: ' + msg);
  }
  ...
  function WriteError(msg) {
    console.error('1-Click Child-Links-Upgrade: ' + msg);
  }
